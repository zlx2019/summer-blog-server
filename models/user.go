...
type User struct {
	...
	UserName string         `json:"userName" gorm:"size:36; uniqueIndex ;comment: 用户名"`
	Password string         `json:"password" gorm:"size:128; not null; comment: 用户密码"`
	Avatar   string         `json:"avatar" gorm:"size:256; comment: 用户头像"`
	...
	Source   RegisterManner `json:"source" gorm:"size:4; comment: 注册来源(1: QQ注册 2: 码云注册 3:github注册 4:邮箱注册)"`
	// 用户发布的所有文章
	// 一对多关系映射
	// 用户(一) --- 文章(多)
