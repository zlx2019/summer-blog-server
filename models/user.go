...
type User struct {
	BaseModel
	NickName string         `json:"nickName" gorm:"type: varchar(42); comment: 用户昵称"`
	...
	Role     Role           `json:"role" gorm:"type:tinyint(4); default:1; comment: 用户角色(1: 管理员 2:普通用户 3:游客)"`
	Source   RegisterManner `json:"source" gorm:"type:tinyint(4); comment: 注册来源(1: QQ注册 2: 码云注册 3:github注册 4:邮箱注册)"`
	// 用户发布的所有文章
	// 一对多关系映射
	// 用户(一) --- 文章(多)
